...
xx = sum(dd.x.^2,2);
rhomax = 2*(dd.yy./dd.nsamps)/mean(xx); % ratio of output to input variance
...
lfun = @(prs)neglogev_ASDspectral_dual(prs,dd,wwnrmtot,CONDTHRESH);  % loss function
...
fminopts = optimset('gradobj','on','Hessian','off','display','off',...
...
[neglogEv,~,mupost,ii,cfdiag] = lfun(hprshat);
% Transform estimate back to space-time domain
Binds = false(size(Bii)); % initialize
Binds(Bii) = ii; % indices in basis
kest = kronmult(Bfft,mupost,Binds); % project to space-time basis by inverse DFT
