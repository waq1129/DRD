function [mu,dmu,k,gamma,lambda] = sparse_learning_lambda(Phi,T,lambda,iters,flag1,flag2,flag3,flag_lambda)
% *************************************************************************
%
% *** PURPOSE ***
...
while (1)
    ...
    if flag_lambda
        lambda = sum((T-Phi*mu).^2)./(N-sum(R_diag));
    end
    
    % *** Check stopping conditions, etc. ***
    k = k+1;
    if (DISPLAY_FLAG) disp(['iters: ',num2str(k),'   num coeffs: ',num2str(m), ...
            '   gamma change: ',num2str(max(abs(gamma - gamma_old))) '    lambda:' num2str(lambda)]); 
    end;
