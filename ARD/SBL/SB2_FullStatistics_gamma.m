...
if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian
    ...
    %
    flag_chol = true;
    Sigmainv = PHI'*PHI*beta + diag(Alpha);
    try
        U		= chol(Sigmainv);
        Ui	= inv(U);
    catch
        [uu,ss,vv] = svd(Sigmainv);
        dss = diag(ss);
        idss = dss>max(dss)*1e-6;
        Ui = vv(:,idss)*diag(1./sqrt(dss(idss)));
        %         SIGMA = vv(:,idss)*diag(1./dss(idss))*uu(:,idss)';
        flag_chol = false;
        %         keyboard
    end
    SIGMA	= Ui * Ui';
...
if flag_chol
    ...
    logdetHOver2 = sum(log(dss));
end
logML			= dataLikely - (Mu.^2)'*Alpha/2 + ...
    sum(log(Alpha))/2 - logdetHOver2 + ...
    sum(prior_alpha(SETTINGS.gam_a, Alpha, SETTINGS.priortype));
