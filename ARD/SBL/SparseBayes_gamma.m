...
function [PARAMETER, HYPERPARAMETER, DIAGNOSTIC] = ...
    SparseBayes_gamma(likelihood_, BASIS, Targets, OPTIONS, SETTINGS)
...
[SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI,beta] = ...
    SB2_FullStatistics_gamma(LIKELIHOOD,BASIS,PHI,Targets,Used, ...
    Alpha,beta,Mu,BASIS_PHI,BASIS_Targets,OPTIONS,SETTINGS);
...
gam_a = SETTINGS.gam_a;
priortype = SETTINGS.priortype;
%
while (~LAST_ITERATION)
    ...
    %%
    [act, Alpha_new] = determine_alpha(Q_out,S_out,gam_a,Used,M_full,CONTROLS.ZeroFactor,priortype);
    ...
    %     index		= Used(iu);
    index = find(act==REEST);
    %     NewAlpha	= S_out(index).^2 ./ Factor(index);
    NewAlpha = Alpha_new(index);
    Alpha_all = zeros(M_full,1); Alpha_all(Used) = Alpha;
    Delta		= (1./NewAlpha - 1./Alpha_all(index)); % Temp vector
    %
    ...
    DeltaML(index)	= (Delta.*(Q_in(index).^2) ./ ...
        ...
        prior_alpha(gam_a, NewAlpha, priortype) - ...
        prior_alpha(gam_a, Alpha_all(index), priortype);
    ...
    if anyToDelete
        ...
        %
        Alpha_all = zeros(M_full,1); Alpha_all(Used) = Alpha;
        DeltaML(index)	= -(Q_out(index).^2 ./ (S_out(index) + Alpha_all(index)) - ...
            log(1 + S_out(index) ./  Alpha_all(index)))/2 - ...
            prior_alpha(gam_a, Alpha_all(index), priortype);
    ...
    if anyToAdd
        ...
        DeltaML(index)	= (quot - 1 - log(quot))/2 + prior_alpha(gam_a, NewAlpha, priortype);
    ...
    if 0%~anyWorthwhileAction
        [deltaLogMarginal, nu_backup]	= max(DeltaML_backup);
        if Action(nu_backup)==ACTION_ADD || Action(nu_backup)==ACTION_REESTIMATE
            nu = nu_backup;
            selectedAction		= Action(nu);
            anyWorthwhileAction	= deltaLogMarginal>0;
        end
    ...
    %
    Phi		= BASIS(:,nu);
    newAlpha = Alpha_new(nu);
    infid = isinf(newAlpha);
    newAlpha(infid)	= S_out(nu(infid))^2 / Factor(nu(infid));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian && ...
        ...
        if abs(deltaLogBeta) > CONTROLS.MinDeltaLogBeta
            ...
            [SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI] = ...
                SB2_FullStatistics_gamma(LIKELIHOOD,BASIS,PHI,Targets,Used,Alpha,beta,...
                Mu, BASIS_PHI,BASIS_Targets,OPTIONS,SETTINGS);
            %
            count					= count + 1;
            logMarginalLog(count)	= logML;
            %
    ...
    %     subplot(412),stem(act)
    %     alphaall = zeros(M_full,1);alphaall(Used) = Alpha;
    %     subplot(413),plot(alphaall)
    %     subplot(414),plot(Alpha_new)
    %     pause(0.1)
    %     drawnow
